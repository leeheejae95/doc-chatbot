...
public class DocumentController {
    ...
    @Operation(
            summary = "문서 업로드",
            description = "파일을 업로드하면 청크 단위로 분할 후 벡터 DB에 저장됩니다."
    )
    @PostMapping(value = "/upload", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<ApiResponse<String>> uploadDocument(
            @Parameter(description = "업로드할 파일", required = true)
            @RequestParam("file") MultipartFile file) {
        log.info("[DocumentController] 문서 업로드 요청 - 파일명: {}, 크기: {}KB", file.getOriginalFilename(), file.getSize() / 1024);
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<DocumentResponse>>> documentList() {
        log.info("[DocumentController] 문서 목록 조회 요청");
        List<DocumentResponse> list = documentService.getDocuments();
    ...
    public ResponseEntity<ApiResponse<Void>> deleteDocument(@PathVariable String id) {
        log.info("[DocumentController] 문서 삭제 요청 - id: {}", id);
